...
memberEmail.addEventListener("input", e => {
 ...
 // -------------------------------------------------------------------------
    ...
    // 인증하기 버튼 클릭 시
    // 입력된 인증번호를 가지고 비동기로 서버에 전달...
    // -> 입력된 인증번호와 실제로 발급된 인증번호가 같은지 비교하기
    //    같으면 1, 아니면 0을 반환
    // 단, 타이머가 00분 00초가 아닐 경우에만 수행
    
    checkAuthKeyBtn.addEventListener("click", () =>{

        if( min === 0 && sec === 0){ // 타이머가 00:00  
            alert("인증번호 입력 제한시간을 초과하였습니다");
            return;
        }

        if(authKey.value.length < 6){ // 인증번호가 제대로 입력 안된 경우
            alert("인증번호를 정확히 입력해주세요 ㅠ");
            return;
        }
        ...
        fetch("/email/checkAuthKey", {
            method : "POST",
            headers : {"Content-Type" : "application/json"},
            body : JSON.stringify(obj) // JS 객체 2개를 JSON으로 변경해서 보냄
        })
        .then( resp => resp.text() )
        .then( result => {
    ...
    const checkPw = () => {
        ...
        if(memberPw.value === memberPwConfirm.value){
            ...
            checkObj.memberPwConfirm = true; // 비밀번호 확인란을 true 로 바꾸고
            return;
        }
        ...
        checkObj.memberPwConfirm = false; // 비밀번호 확인을 false
    };
    ...
    memberPw.addEventListener("input", e=> {
        ...
        if(!regExp.test(inputPw)){ // 유효하지 않은 경우
        ...
        // 비밀번호 확인란에 값이 작성되어 있을 때만 비교해줄거다
    ...
    signUpForm.addEventListener("submit", e => {
        ...
        for(let key in checkObj) { // checkObj 요소의 key 값을 순서대로 꺼내온다.
            ...
            if( !checkObj[key] ){ // false인 경우 (유효하지 않은 경우)
                ...
                e.preventDefault(); // form태그 기본 이벤트 ( 제출 )막기
